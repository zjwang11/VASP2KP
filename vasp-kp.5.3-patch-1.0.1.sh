...
############################################################################################################
# Patch Instructions
# vasp-kp.5.3-patch-1.0.1.sh
# compile the script vasp-kp.5.3
...
module song_data
 ...
 contains
    ...
    subroutine song_getinp(IO)
        ...
        ! read in------------------------------
        !
        open(unit=201, file='INCAR.song', status='old', iostat=stat)
        if (stat/=0 .and. IO%IU0>=0) then
            write(IO%IU0,*) 'Can not open file INCAR.song !!!!!!!!!!!!!!!!!!!!!!!!!!!!'
            stop
        endif
        !
        read(201, vmat_para ,iostat=stat)
        if (stat/=0 .and. IO%IU0>=0) then
            write(IO%IU0,*) 'Can not read "vmat_para" list in file INCAR.song !!!!!!!!!!!!!!!!!!!!!!!!!!!!'
            stop
        endif
        !
        close(201)
...
subroutine vmat_top(IO, T_INFO, LATT_CUR, KPOINTS, WDES, GRID, W, LMDIM, &
    ...
    case(14)    ! wilson loop
        ...
        IF(IO%IU6>=0) call vmat_print_maxtrix(matrix, vmat_nbands, " Berry's phase in 2pi", &
                           trim(adjustl(vmat_name)), WDES%ISPIN)
        !
    case(15)    ! \sigma,  i.e. pauli matrix and ! pi = p + \alpha^2/4 * sigma x \nabla V
...
# judge if vasp-kp.5.3 already exists
if [ -d "vasp-kp.5.3" ]; then
    echo "The file folder 'vasp-kp.5.3' already exists, if you continue the installation, this folder will be deleted. Do you want to continue? [y/n]"
    read flag
    while [ 0 ]; do
...
echo "Finishing modifying vasp into vasp-kp!"
echo "vasp-kp source file is in the file folder vasp-kp.5.3, do you want to compile it right now? [y/n]"

# ask if the user want to compile vasp-kp right now
read flag_2
while [ 0 ]; do
...
# rename vasp to vasp-kp
mv vasp vasp-kp

# path of the file folder of vasp-kp
current_path=$(pwd)
...
# print the path
echo " "
echo "Finishing installing vasp-kp!"
echo "The path of vasp-kp: $current_path/vasp-kp"
