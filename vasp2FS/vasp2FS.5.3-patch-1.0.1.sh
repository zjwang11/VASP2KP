...
# step 4: run this patch: bash vasp2FS.5.3-patch-1.0.1.sh
...
if [ -d "vasp2FS.5.3" ]; then
    echo "The file folder 'vasp2FS.5.3' already exists, if you continue the installation, this folder will be deleted. Do you want to continue? [y/n]"
    read flag
    while [ 0 ]; do
...
sed -i 's/-o vasp \(.*\)/-o vasp2FS \1/' $makefile

echo "Finish modifying vasp into vasp2FS!"
echo "vasp2FS source file is in the file folder vasp2FS.5.3, do you want to compile it right now? [y/n]"
...
echo "The path of vasp2FS: $current_path/vasp2FS"
