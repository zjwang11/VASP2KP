...
# step 4: run this patch: bash vasp2mat.5.3-patch-1.0.2.sh
...
subroutine vmat_top(IO, T_INFO, LATT_CUR, KPOINTS, WDES, GRID, W, LMDIM, &
    ...
    !write(*,"(2I5)") q,vmat_nbands;stop !wzj
        ...
        open(1315, status="replace" )!eig
        IF (WDES%NCDIJ == 4) open(1316, status="replace") !sigma
        open(1317, status="replace") !vmat

        IF(WDES%NCDIJ/=4) THEN
            if(IO%IU0>=0) write(IO%IU0, *) 'pauli matrix will only be computed in non-collinear case'
        ENDIF
!
        !vmat_k =  KPOINTS%NKPTS 
        if( vmat_k.gt. KPOINTS%NKPTS ) STOP "Wrong: vmat_k"

        IF (WDES%NCDIJ == 4) THEN
            write(91, '("% Total k-number:",8I5)')  vmat_k
            write(91, '("% Eigenvalues         are output in fort.1315 in units of eV")')
            write(91, '("% Sigma Matrices(MAT) are output in fort.1316 in units of hbar/bohr")')
            write(91, '("% Velocity (Pi) MAT   are output in fort.1317 in units of hbar/bohr")')
    !
            write(1315,'(2I10)') vmat_nbands,vmat_k!KPOINTS%NKPTS-1
            write(1316,'(2I10)') vmat_nbands,vmat_k!KPOINTS%NKPTS-1
            write(1317,'(2I10)') vmat_nbands,vmat_k!KPOINTS%NKPTS-1
            write(   *,'(2I10)') vmat_nbands,vmat_k!KPOINTS%NKPTS-1
                                    wzjk = vmat_k
            do vmat_k=1,wzjk
            write(   *,'(A,I5)') 'k=',vmat_k
    !write(1315,"(I5,80000E14.6)") vmat_k,(REAL( W%CELTOT(ii,vmat_k,1), KIND=q), ii=1,vmat_nbands)
            write(1315,"(I5,80000E14.6)") vmat_k,(REAL( W%CELTOT(vmat_bands(ii),vmat_k,1), KIND=q),ii=1,vmat_nbands)
            write(1316,'(A,I5)') 'k=',vmat_k
    ! x
            call vmat_overlap_matrix(matrix, vmat_nbands, vmat_bands, WDES, W, CQIJ,LMDIM, GRID, sig1)
            write(1316,"(8E14.6)") matrix(:,:,1) !wzj
    !
    ! y
            call vmat_overlap_matrix(matrix, vmat_nbands, vmat_bands, WDES, W, CQIJ,LMDIM, GRID, sig2)
            write(1316,"(8E14.6)") matrix(:,:,1) !wzj
    ...
    ! z
            ...
            write(1316,"(8E14.6)") matrix(:,:,1) !wzj
    !
            write(1317,'(A,I5)') 'k=',vmat_k
    ! x
            call vmat_pi_matrix(matrix, vmat_nbands, vmat_bands, WDES, W, LATT_CUR%B, T_INFO, P, LMDIM, 1)
            write(1317,"(8E14.6)") matrix(:,:,1) !wzj
    !
    ! y
            call vmat_pi_matrix(matrix, vmat_nbands, vmat_bands, WDES, W, LATT_CUR%B, T_INFO, P, LMDIM, 2)
            write(1317,"(8E14.6)") matrix(:,:,1) !wzj
  ...
  ! logical, optional :: rev_in
   ...
   !cpt(:) = 0.0_q
    ...
    do m=1, WDES%NGVECTOR(k2)
        do m_=1, WDES%NGVECTOR(k1)
            !
            if ( WDES%IGX(m,k2)+dk(1)==WDES%IGX(m_,k1) .and. &
                 WDES%IGY(m,k2)+dk(2)==WDES%IGY(m_,k1) .and. &
                 WDES%IGZ(m,k2)+dk(3)==WDES%IGZ(m_,k1) ) then
                !
                ng2(m_)=m; tf1(m_)=.true.
                !
                exit ! exit the loop
                !
            endif
        ...
        do m_=1, WDES%NGVECTOR(k1)
           IF(tf1(m_)) THEN
               ...
               do isp =0, WDES%NRSPINORS-1
                   mm_= m_+ isp_*WDES%NGVECTOR(k1)
                   mm = ng2(m_) + isp *WDES%NGVECTOR(k2)
                   !
...
subroutine vmat_rotation(cc, b1, b2, k1, k2, dk_in, rot_in, tau_in, sig_in, rev_in, WDES, W, sp, NONLR_S, NONL_S, GRID, CQIJ, LATT_CUR, KPOINTS)
    ...
    do m=1, WDES%NGVECTOR(k2)
        ...
        do m_=1, WDES%NGVECTOR(k1)
            !
            if ( Gpt(1)==WDES%IGX(m_,k1) .and. &
                 Gpt(2)==WDES%IGY(m_,k1) .and. &
                 Gpt(3)==WDES%IGZ(m_,k1) ) then
                !
                ! phase factor
...
subroutine vmat_overlap(cc, kp1, ii, kp2, jj, WDES, W, CQIJ, LMDIM, GRID, sp, pauli)
    ...
    IF( present(pauli) ) THEN
        if (kp1==kp2) then
            ...
            do m =1, WDES%NGVECTOR(kp1)
            do m_=1, WDES%NGVECTOR(kp2)
                !
                if ( WDES%IGX(m ,kp1) == WDES%IGX(m_,kp2) .and. &
                     WDES%IGY(m ,kp1) == WDES%IGY(m_,kp2) .and. &
                     WDES%IGZ(m ,kp1) == WDES%IGZ(m_,kp2) ) then
                    !
                    do isp =0, WDES%NRSPINORS-1
                    do isp_=0, WDES%NRSPINORS-1
                        mm = m +  isp*WDES%NGVECTOR(kp1)
                        mm_= m_+ isp_*WDES%NGVECTOR(kp2)
                        overlap_p = overlap_p +  conjg( W%CPTWFP(mm ,ii, kp1, sp)) &
                                                      * W%CPTWFP(mm_,jj, kp2, sp)  &
                                                      * pauli(isp,  isp_)
                    enddo
...
# judge if vasp2mat.5.3 already exists
if [ -d "vasp2mat.5.3" ]; then
    echo "The file folder 'vasp2mat.5.3' already exists, if you continue the installation, this folder will be deleted. Do you want to continue? [y/n]"
    read flag
    while [ 0 ]; do
...
line_number=$(grep -n "IF ( EXXOEP==1 ) THEN" "main.F" | sed -n 's/^\([0-9]\+\):.*$/\1/p')
if [ -z "$line_number" ]; then
    echo "Error: cannot find the right line in main.F!!!"
    exit
fi
...
line_number=$(grep -n "invmc2=hbar^2/2(m_e c)^2 in eV/A^2" "relativistic.F" | sed -n 's/^\([0-9]\+\):.*$/\1/p')
if [ -z "$line_number" ]; then
    echo "Error: cannot find the right line in relativistic.F!!!"
    exit
fi
...
line_number=$(expr $line_number + 1)
sed -i "${line_number}r /dev/stdin" relativistic.F <<< "      C_ = C*cfactor"
line_number=$(expr $line_number + 3)
sed "${line_number}s/C/C_/g" relativistic.F > t && mv t relativistic.F
sed "${line_number}s/INVMC_2/INVMC2_/g" relativistic.F > t && mv t relativistic.F
...
line_number=$(grep -n "constant.o jacobi.o   main_mpi.o  scala.o" $makefile | sed -n 's/^\([0-9]\+\):.*$/\1/p')
if [ -z "$line_number" ]; then
    echo "Error: cannot find the right line in ${makefile}!!!"
    exit
fi
awk -v line="$line_number" -v insert="song_data.o " -v search="constant.o" '
{
    if (NR == line) {
        sub(search, insert search)
    }
    print
}
' $makefile > temp.txt && mv temp.txt $makefile

line_number=$(grep -n "vasp: " $makefile | sed -n 's/^\([0-9]\+\):.*$/\1/p')
if [ -z "$line_number" ]; then
    echo "Error: cannot find the right line in ${makefile}!!!"
    exit
fi
...
sed -i "${line_number}r /dev/stdin" $makefile <<< "         song_vmat.o "

sed "${line_number}s/$/ \\\/" $makefile > temp.txt && mv temp.txt $makefile

echo "Finishing modifying vasp into vasp2mat!"
echo "vasp2mat source file is in the file folder vasp2mat.5.3, do you want to compile it right now? [y/n]"
