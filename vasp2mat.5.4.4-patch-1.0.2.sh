...
############################################################################################################
# Patch Instructions
# vasp2mat.5.4.4-patch-1.0.2.sh
# created by Sheng Zhang, Institute of Physics, Chinese Academy of Sciences
# compile the script vasp2mat.5.4.4
# step 1: copy the folder vasp.5.4.4 in the same folder as this script
# step 2: create and revise makefile in vasp.5.4.4 so that vasp_ncl can be compiled successfully (no need to compile)
# step 3: run this patch: bash vasp2mat.5.4.4-patch-1.0.2.sh
...
# judge if vasp2mat.5.4.4 already exists
if [ -d "vasp2mat.5.4.4" ]; then
    echo "The file folder 'vasp2mat.5.4.4' already exists, if you continue the installation, this folder will be deleted. Do you want to continue? [y/n]"
    read flag
    while [ 0 ]; do
        if [ "$flag" == "n" ] || [ "$flag" == "N" ]; then
            exit
        elif [ "$flag" == "y" ] || [ "$flag" == "Y" ]; then
            rm -rf vasp2mat.5.4.4
            break
        else
            read flag
        fi
...
# prepare the files
mkdir vasp2mat.5.4.4
cd vasp2mat.5.4.4

mkdir bin
mkdir build
cp ../vasp.5.4.4/makefile.include .
cp -r ../vasp.5.4.4/src .
...
line_number=$(grep -n "IF (.NOT. LJ_ONLY .AND. .NOT. LCHI) THEN" "main.F" | sed -n 's/^\([0-9]\+\):.*$/\1/p')
if [ -z "$line_number" ]; then
    echo "Error: cannot find the right line in main.F!!!"
    exit
fi
...
# revise .object
line_number=$(
    grep -n "constant.o" .objects \
    | grep -v 'ml_ff_constant.o' \
    | sed -n 's/^\([0-9]\+\):.*/\1/p' \
    | head -n1                       # 只取首行
)
...
line_number=$(
    grep -n "elphon.o" .objects \
    | sed -n 's/^\([0-9]\+\):.*/\1/p' \
    | head -n1                       # 只取首行
)

# line_number=$(grep -n "elphon.o" ".objects" | sed -n 's/^\([0-9]\+\):.*$/\1/p')
if [ -z "$line_number" ]; then
    echo "Error: cannot find the right line in .objects!!!"
    exit
fi
...
# revise makefile
if [ -e "../vasp.5.4.4/makefile" ]; then
  makefile="makefile"
elif [ -e "../vasp.5.4.4/Makefile" ]; then
  makefile="Makefile"
else
  echo "File 'Makefile' not exists in vasp.5.4.4!"
  exit
fi

cp  ../vasp.5.4.4/$makefile ./
...
echo "vasp2mat source file is in the file folder vasp2mat.5.4.4, do you want to compile it right now? [y/n]"
